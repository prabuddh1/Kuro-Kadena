(module kuro-compliance GOV
  ;; governance & oracle keysets provided in deploy YAML "data"
  (defconst GOV (read-keyset "GOV"))
  (defconst ORACLE (read-keyset "ORACLE"))

  ;; simple tables
  (deftable kyc
    { account:string, status:string, ts:time })

  (deftable mint_approvals
    { id:string, account:string, amount:decimal, exp:time, used:bool, ts:time })

  (deftable redeem_approvals
    { id:string, account:string, amount:decimal, bankref:string, exp:time, used:bool, ts:time })

  (deftable events
    { id:string, typ:string, payload:string, ts:time })
  ...
  (defun consume-mint:string (id:string)
    ...
    (let ((m (read mint_approvals id)))
      ...
      (write mint_approvals id
        {"id":id,"account":(at "account" m),"amount":(at "amount" m),
         "exp":(at "exp" m),"used": true, "ts": (now)})
      "ok"))
  ...
  (defun consume-redeem:string (id:string)
    ...
    (let ((r (read redeem_approvals id)))
      ...
      (write redeem_approvals id
        {"id":id,"account":(at "account" r),"amount":(at "amount" r),
         "bankref":(at "bankref" r),"exp":(at "exp" r),"used": true, "ts": (now)})
      "ok"))
